...
1. Mirror changes from tutorial
1. `src/test/resources/META-INF/native-image/helloworld/reflect-config.json`, replace `helloworld.App` with `helloworld.S3GraalVM`
1. Add the following to `build.gradle`: `implementation 'com.amazonaws:aws-lambda-java-events:3.11.0'` and `implementation 'com.amazonaws:aws-lambda-java-runtime-interface-client:2.1.1'`
1. `Makefile` rename target to `build-S3GraalVM`
1. `template.yaml` update `runtime` to `provided.al2`
...
aws lambda invoke --function-name <replace-with-your-lambda-identifier> outfile --region ap-southeast-2
sam logs --stack-name sam-graalvm17 --region ap-southeast-2
```
...
```bash
sam-graalvm17$ sam build
sam-graalvm17$ sam deploy --guided
```
...
```bash
sam-graalvm17$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-graalvm17$ sam local start-api
sam-graalvm17$ curl http://localhost:3000/
```
...
```bash
sam-graalvm17$ sam logs -n HelloWorldFunction --stack-name <Name-of-your-deployed-stack> --tail
```
...
```bash
sam-graalvm17$ cd HelloWorldFunction
